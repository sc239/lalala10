...
def sample_on_grid(model: VAE, latent0_min: float, latent0_max: float, latent1_min: float, latent1_max: float,
    ...
    step_size = np.array([(latent0_max - latent0_min) /
                         grid_size, (latent1_max - latent1_min) / grid_size])
    ...
    mesh_grid_reshaped = mesh_grid.reshape(grid_size ** 2, 2)
    decoded = model.decode(mesh_grid_reshaped)
    # END TODO ########################

    # the decoded shape is (grid_size ** 2, 1, 28, 28)
    # reshape it to recreate the original grid structure
    _, c, h, w = decoded.shape
    decoded_reshaped = decoded.reshape(
        grid_size, grid_size, c, h, w).cpu().numpy()
...
def plot_latent_space(model: VAE, test_loader: data.DataLoader, device: str = "cpu") -> None:
    ...
    class_stddev_0 = np.bincount(
        labels, weights=stddev[:, 0]) / np.bincount(labels)
    class_stddev_1 = np.bincount(
        labels, weights=stddev[:, 1]) / np.bincount(labels)
    mean_stddev = np.stack((class_stddev_0, class_stddev_1))
    print(
        f"Per class mean of estimated std deviation: {mean_stddev.shape}\n{mean_stddev}")
    ...
    mean_mu_scaled = 28 / 2 + \
        scale.reshape(2, 1) * (mean_mu - latent_min.reshape(2, 1))
