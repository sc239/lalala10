...
class VAE(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, in_height: int = 28, in_width: int = 28, hidden_size: int = 100,
        ...
        self.hidden_enc = nn.Linear(input_size, hidden_size)
        self.mu = nn.Linear(hidden_size, latent_size)
        self.logvar = nn.Linear(hidden_size, latent_size)
        self.hidden_dec = nn.Linear(latent_size, hidden_size)
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Estimate the mean and the log of the variance from the hidden representation, i.e. use two linear layers
        ...
        # START TODO ########################
        hidden = self.activation(self.hidden_enc(x))
        mu = self.mu(hidden)
    ...
    def reparameterize(self, mu, logvar) -> torch.Tensor:
        ...
        if self.training:
            ...
            sampled = mu + sd * torch.randn_like(mu)
            return sampled
            # END TODO ########################
        else:
            # START TODO ########################
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # END TODO ########################
        hidden = self.activation(self.hidden_dec(z))
