...
def test_train_gan():
    ...
    errG, D_G_z2 = update_generator(
        discriminator, generator, fake_images, label, optimizerG)
    ...
    # Test outputs
    true_ud_output = np.array(
        [0.5513347387313843, 0.5765248537063599, 0.5437031388282776, 0.41870588064193726])
    ...
    np.testing.assert_allclose(
        true_ud_output, ud_output, err_msg=err_msg_d, rtol=1e-6)
    np.testing.assert_allclose(
        true_ug_output, ug_output, err_msg=err_msg_g, rtol=1e-6)
